...
class JournalStorage {
  ...
  try {
    ...
    print('Sentiment: ${entry.sentiment} (type: ${entry.sentiment.runtimeType})');
    ...
    final response = await _supabase.from('journals').insert(insertData).select();
  ...
  static Future<void> update(JournalEntry entry) async {
    try {
      ...
      await _supabase.from('journals').update({
        ...
        'created_at': entry.at.toUtc().toIso8601String(),
        'is_private': entry.isPrivate,
      }).eq('id', int.parse(entry.id)).eq('user_id', userId);
    } catch (e) {
